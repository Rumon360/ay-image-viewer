...
export default function App() {
  ...
  const [modalState, setModalState] = useState<{ image: SelectedImage; siblings: SelectedImage[] } | null>(null)
  const [collagePanelOpen, setCollagePanelOpen] = useState(false)
  const [completeOnly, setCompleteOnly] = useState(false)

  const filteredRecords = completeOnly && allImageColumns.length > 0
    ? records.filter(r => allImageColumns.every(col => r.images[col]))
    : records

  const pagination = usePagination(filteredRecords.length)
  const pageRecords = filteredRecords.slice(pagination.startIndex, pagination.endIndex)
  ...
  const allPageSelected = pageImages.length > 0 && pageImages.every(img => selection.isSelected(img))
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-void)' }}>
      ...
      >
        <div className="max-w-screen-xl mx-auto px-6 py-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div
              ...
              style={{ background: 'rgba(0,255,255,0.08)', border: '1px solid rgba(0,255,255,0.20)' }}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#00ffff" strokeWidth="1.75" strokeLinecap="round">
                <rect x="3" y="3" width="18" height="18" rx="2" />
                <circle cx="8.5" cy="8.5" r="1.5" />
                <polyline points="21 15 16 10 5 21" />
              </svg>
            </div>
            <span className="text-sm font-medium" style={{ fontFamily: 'var(--font-mono)', color: 'rgba(255,255,255,0.85)' }}>
              ay-image-viewer
            </span>
          </div>
        ...
        {hasData && errors.length > 0 && (
          <div className="max-w-screen-xl mx-auto px-6 pb-3 flex flex-col gap-1">
            {errors.map((err, i) => (
              <div
                key={i}
              ...
              >
                ...
                <span><span>{err.file}</span>: {err.message}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                style={{
                  fontFamily: 'var(--font-mono)',
                  background: completeOnly ? 'rgba(0,255,255,0.08)' : 'transparent',
                  borderColor: completeOnly ? 'rgba(0,255,255,0.30)' : 'rgba(255,255,255,0.10)',
              ...
              >
                <span
                  style={{
                    ...
                    background: completeOnly ? 'rgba(0,255,255,0.25)' : 'rgba(255,255,255,0.08)',
                    border: `1px solid ${completeOnly ? 'rgba(0,255,255,0.4)' : 'rgba(255,255,255,0.12)'}`,
                    transition: 'all 150ms ease-out', flexShrink: 0, position: 'relative',
                  }}
                >
                  <span style={{
                    width: 10, height: 10, borderRadius: '50%',
                    background: completeOnly ? '#00ffff' : 'rgba(255,255,255,0.35)',
                    position: 'absolute',
            ...
            <DataTable
              ...
              onImageClick={(img, siblings) => setModalState({ image: img, siblings })}
              onImageSelect={selection.toggleImage}
              isSelected={selection.isSelected}
              onSelectAllPage={(imgs, checked) => checked ? selection.selectAll(imgs) : selection.deselectAll(imgs)}
      ...
      {modalState && (
        <ImageModal
          ...
          onNavigate={img => setModalState(s => s ? { ...s, image: img } : null)}
        />
      )}
